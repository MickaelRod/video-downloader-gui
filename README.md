...
- **EN**: Bilingual GUI (English/French), two download engines (yt-dlp for streaming platforms with video/audio format selection, ffmpeg for direct video file links), audio-only downloads, link validation, duplicate filename protection, editable destination folder, live progress bar with cancel button, automatic retry past some bot-check walls (via deno, no browser cookies involved), download summary (size, duration, location, elapsed time).
